...
if not int(os.environ.get("DEBUG")):
    import logging
    logging.basicConfig(filename='logs/python_index.log',
                        level=logging.INFO,
...
TOKEN = "a"
if not int(os.environ.get("DEBUG")):
    TOKEN = get_docker_secrets("TOKEN")

# Layout for the navigation bar
NAVIGATION_BAR = Markup(f"""<nav class="stroke">
    <ul>
        <li class="dropdown">
            <a href="javascript:void(0)" class="dropbtn">Seleccionar temporada</a>
            <div class="dropdown-content">
                <a href="/{TOKEN}/select_season?season=0">Temporada 1</a>
                <a href="/{TOKEN}/select_season?season=1">Temporada 2</a>
                <a href="/{TOKEN}/select_season?season=2">Temporada 3</a>
            </div>
        </li>
...
def update_results(request_info, season):
    ...
    new_game = TABLES[f"Games_history_{season}"](timestamp=datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                                      winner1 = db_w1.name, winner1_percentage = round(expected_win_winner1*100, 2),
                                      winner1_old_elo = elo_winner1, winner1_new_elo = new_elo_winner1,
                                      winner2 = db_w2.name, winner2_percentage = round(expected_win_winner2*100, 2),
                                      winner2_old_elo = elo_winner2, winner2_new_elo = new_elo_winner2,
                                      loser1 = db_l1.name, loser1_percentage = round(expected_win_loser1*100, 2),
                                      loser1_old_elo = elo_loser1, loser1_new_elo = new_elo_loser1,
                                      loser2 = db_l2.name, loser2_percentage = round(expected_win_loser2*100, 2),
                                      loser2_old_elo = elo_loser2, loser2_new_elo = new_elo_loser2)
...
def delete_last_game(season):
    last_game = TABLES[f"Games_history_{season}"].query.order_by(TABLES[f"Games_history_{season}"].id.desc()).first()
...
def view_season():
    ...
    # Redirect to the main scoreboard page
    return render_template('score_season.html', league_name=LEAGUE_NAME,
...
def scoreboard():
    ...
    # Set the selected season in the user's session to 2,
    ...
    return render_template('scoreboard.html', league_name=LEAGUE_NAME,
                            navigation_bar=NAVIGATION_BAR,
                            players=enumerate(players, start=1))
...
def go_home_update():
    ...
    # Generate Jinja2 template for the scoreboard
    return render_template('score_update.html', league_name=LEAGUE_NAME,
                           navigation_bar=NAVIGATION_BAR, 
                           players=enumerate(players, start=1), 
...
def game_history():
    ...
    return render_template('game_history.html',  league_name=LEAGUE_NAME,
                           navigation_bar=NAVIGATION_BAR,
...
def add_player():
    ...
    if request.method == 'POST':
        ...
        if TABLES[f"Season_{DEFAULT_SEASON}"].query.filter_by(name=new_player_name).first():
            message_new_player = "Error: There was already a player with that name"
        new_player = TABLES[f"Season_{DEFAULT_SEASON}"](name=new_player_name,score=1000, wins=0, losses=0, total=0)
